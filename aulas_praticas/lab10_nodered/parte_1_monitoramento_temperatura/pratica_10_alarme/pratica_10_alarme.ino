...
void publicarBME280() {
  ...
  char temperatura[10], umidade[10], luminosidadeStr[10];
  dtostrf(temp, 5, 2, temperatura);
  dtostrf(hum, 5, 2, umidade);
  snprintf(luminosidadeStr, sizeof(luminosidadeStr), "%d", luminosidade);

  mqttClient.publish("casa/temperatura", temperatura);
  mqttClient.publish("casa/umidade", umidade);
  mqttClient.publish("casa/luminosidade", luminosidadeStr);

  Serial.print("Temp: "); Serial.print(temperatura);
  Serial.print(" | Hum: "); Serial.print(umidade);
  Serial.print(" | Lum: "); Serial.println(luminosidadeStr);
}
